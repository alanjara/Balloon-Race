...
namespace UnityStandardAssets.Characters.ThirdPerson {
    ...
    public class BaboonMob : MonoBehaviour {
        [SerializeField]
        ...
        void Update() {
            ...
            to_target.y = 0;
            to_target.Normalize();
            MobMove(to_target, false, try_jump);
            try_jump = false;
        }
        public void OnTriggerEnter(Collider coll) {
            ...
            if (coll.name == target.name) {
                ...
                if (current < numpoints)
                    target = transform.parent.Find(string.Format("{0}{1}", waypointname, current)).gameObject;
                else if(loop){
                    current = 0;
                    target = transform.parent.Find(string.Format("{0}{1}", waypointname, current)).gameObject;
                }
        ...
        public void MobMove(Vector3 move, bool crouch, bool jump) {
